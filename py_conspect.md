...
print(переменная)                    вывод вертикально
print(переменная, end = ", ")        вывод горизонтально
```
...
* Форматирование дрoбного числа без округления:
...
Буква ***f*** указывает на то, что мы создаем ***f-строку*** — шаблон, в который с помощью фигурных скобок подставляются
значения переменных. На выходе получается обычная строка.
...
```
* заполненный список случайными целыми числами:
